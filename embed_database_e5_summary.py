...
class CustomSentenceTransformerEmbeddingFunction(EmbeddingFunction):
    def __init__(self):
        ...
        請轉換以下法條：
        """
    def find_first_article_index(self, sections: list[str]) -> int:
        for i, section in enumerate(sections):
            match = re.search(r'第\s*(\d+(?:-\d+)?)\s*條', section)
            if match:
                try:
                    number_parts = match.group(1).split('-')
                    if all(0 <= int(num) <= 9999 for num in number_parts):
                        return i
                except ValueError:
                    continue
        return -1

    def get_section_weights(self, text: str) -> float:
        # Split the text into sections
        sections = [s.strip() for s in text.split(',')]
        
        # Find the index of the first article pattern
        first_article_idx = self.find_first_article_index(sections)
        
        # If no article pattern found or it's the first section, treat all sections as equal
        if first_article_idx == -1:
            return 1.0
            
        # Check if this text is part of the important sections (before first article)
        for i, section in enumerate(sections):
            if section in text and i < first_article_idx:
                return 3.0
    ...
    def __call__(self, input: list[str]) -> list[list[float]]:
        ...
        # Get base embeddings
        base_embeddings = self.model.encode(formatted_inputs,
                                          convert_to_tensor=True,
                                          normalize_embeddings=True)
        
        # Calculate weights for each input
        weights = [self.get_section_weights(text) for text in input]
        
        # Apply weights to embeddings
        weights_tensor = torch.tensor(weights, device=base_embeddings.device).unsqueeze(1)

        weighted_embeddings = base_embeddings * weights_tensor
        
        # Normalize the weighted embeddings
        normalized_embeddings = torch.nn.functional.normalize(weighted_embeddings, p=2, dim=1)
